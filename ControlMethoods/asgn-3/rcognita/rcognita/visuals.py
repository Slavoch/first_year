...
class Animator3WRobot(Animator):
    ...
    def __init__(self, objects=[], pars=[]):
        ...
        self.est_state_full_init = state_full_init
        self.t1 = t1
        ...
        alpha0 = state_init[2]
        v0 = state_full_init[3]
        omega0 = state_full_init[4]
        
        est_xCoord0 = self.est_state_full_init[0]
        est_yCoord0 = self.est_state_full_init[1]
        est_alpha0 = self.est_state_full_init[2]
        est_v0 = self.est_state_full_init[3]
        est_omega0 = self.est_state_full_init[4]
        
        
        alpha_deg0 = alpha0 / 2 / np.pi
        est_alpha_deg0 = est_alpha0 / 2 / np.pi
        ...
        (self.line_traj_est,) = self.axs_xy_plane.plot(est_xCoord0, est_yCoord0, "b--", lw=0.5)
        
        self.robot_marker = RobotMarker(angle=alpha_deg0)
        self.robot_marker_est = RobotMarker(angle=est_alpha_deg0)
        ...
        #    t0, alpha0, "r-", lw=0.5, label=r"$\alpha$ [rad]"
        #)
        ...
        (self.my_line,) = self.axs_sol.plot([t0], v0, "g-", lw=0.85, label='linear velocity')
        #(self.my_line_est,) = self.axs_sol.plot([t0], est_v0, "g--", lw=0.85, label='linear velocity estimation')

        (self.my_line2,) = self.axs_sol.plot([t0], omega0, "k-", lw=0.85, label='angular velocity')
        #(self.my_line2_est,) = self.axs_sol.plot([t0], est_omega0, "k--", lw=0.85, label='angular velocity estimation')
        ...
        #text_accum_obj = r"$\int \mathrm{{Stage\,obj.}} \,\mathrm{{d}}t$ = {accum_obj:2.3f}".format(
         #   accum_obj=0
        #)
        ...
        (self.line_stage_obj,) = self.axs_cost.plot(
            t0, 0, "r-", lw=0.5, label="estimation error"
        )
        self.axs_cost.legend(fancybox=True, loc="upper right")

        # Control
        self.axs_ctrl = self.fig_sim.add_subplot(
            224,
            autoscale_on=False,
            xlim=(t0, t1),
            ylim=(1.1 * np.min([-10, -10]), 1.1 * np.max([10, 10])),
            xlabel="t [s]",
        )
        self.axs_ctrl.plot([t0, t1], [0, 0], "k--", lw=0.75)  # Help line
        first_three_out = np.array([xCoord0, yCoord0, alpha0]) #est_xCoord0, est_yCoord0, est_alpha0])
        est_first_three_out = np.array([est_xCoord0, est_yCoord0, est_alpha0])
        
        self.axs_ctrl.set_prop_cycle(color=['red', 'green', 'blue'])
        
        self.lines_coord = self.axs_ctrl.plot(t0, to_col_vec(first_three_out).T, lw=0.5)
        self.est_lines_coord = self.axs_ctrl.plot(t0, to_col_vec(est_first_three_out).T, lw=0.5, linestyle='dashed')
        self.axs_ctrl.legend(
            chain(iter(self.lines_coord), iter(self.est_lines_coord)), ("x", "y", "alpha","x_est", "y_est", "alpha_est"), fancybox=True, loc="upper right"
        )
        ...
        self.lines = cLines(
            ...
            lines_coord=self.lines_coord,
            est_lines_coord = self.est_lines_coord
        )
    ...
    def init_anim(self):
        ...
        est_xCoord0 = state_init[0]
        est_yCoord0 = state_init[1]
        
        #scatter_sol_snapshot = self.axs_xy_plane.scatter(
        #    xCoord0, yCoord0, marker=self.robot_marker.marker, s=400, c="b"
        #)
        #
        #scatter_sol_snapshot_est = self.scatter_sol_est = self.axs_xy_plane.scatter(
        #    xCoord0, yCoord0, marker=self.robot_marker_est.marker, s=400, c="r"
        #)
        
        self.scatter_sol = self.axs_xy_plane.scatter(
            xCoord0, yCoord0, marker=self.robot_marker.marker, s=400, c="b", label="ground truth"
        )
        self.scatter_sol_est = self.axs_xy_plane.scatter(
            xCoord0, yCoord0, marker=self.robot_marker_est.marker, s=400, c="r", label="estimation"
        )
        self.axs_xy_plane.legend(borderpad=0.9, labelspacing=1.8, bbox_to_anchor=(1., 1.))
        self.run_curr = 1
    ...
    def animate(self, k):
        ...
        noisy_observation = observation +  np.random.randn(len(observation)) *10
        est_state_full = self.state_estimator.compute_estimate(noisy_observation, action)
        ...
        est_v = est_state_full[3]
        est_omega = est_state_full[4]
        
        stage_obj = np.sqrt(abs((est_v - v)**2 + (est_omega - omega)**2))
        ...
        # Control
        for (line, action_single) in zip(self.lines_coord, [xCoord, yCoord, alpha, est_xCoord, est_yCoord, est_alpha]):
...
class Animator3WRobotNI(Animator):
    ...
    def __init__(self, objects=[], pars=[]):
        ...
        self.axs_ctrl.plot([t0, t1], [0, 0], "k--", lw=0.75)  # Help line
        self.lines_ctrl = self.axs_ctrl.plot(t0, to_col_vec([est_xCoord0, est_yCoord0, est_xCoord0]).T, lw=0.5)
        self.axs_ctrl.legend(
...
class Animator2Tank(Animator):
    ...
    def __init__(self, objects=[], pars=[]):
        ...
        self.axs_sol.plot([t0, t1], [0, 0], "k--", lw=0.75)  # Help line
        self.axs_sol.plot(
            [t0, t1], [level_target[0], level_target[0]], "b--", lw=0.75
        )  # Help line (target)
        self.axs_sol.plot(
            [t0, t1], [level_target[1], level_target[1]], "r--", lw=0.75
        )  # Help line (target)
